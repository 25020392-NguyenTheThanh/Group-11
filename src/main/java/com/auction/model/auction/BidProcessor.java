...
class BidProcessor {
    ...
    synchronized void execute(Bidder bidder, double amount, boolean isAutoBid)
        ...
        BidUpdateData upd = new BidUpdateData(auction.getId(), auction.getCurrentHighestBid(), bidder.getUsername(), auction.getBidHistory().size());
        ...
        if (AuctionServer.getInstance() != null)
            AuctionServer.getInstance().broadcast(new Notification("BID_UPDATE",
                    new BidUpdateData(auction.getId(), auction.getCurrentHighestBid(), bidder.getUsername(), auction.getBidHistory().size())));
    }
}