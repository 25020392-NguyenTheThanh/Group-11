...
public class RequestProcessor {
    ...
    private static Response handleDeleteItem(Request request, ClientHandler handler) {
        ...
        if (success) return Response.ok("Xóa sản phẩm thành công");
        else return Response.error("Không thể xóa sản phẩm khỏi cơ sở dữ liệu");
    }
    ...
    private static Response handleUpdateItem(Request request, ClientHandler handler) {
        ...
        if (existingItem == null) return Response.error("Sản phẩm không tồn tại");
    ...
    private static Response handleTopUp(Request request, ClientHandler handler) {
        ...
        if (!(user instanceof Bidder)) return Response.error("Chỉ người mua (Bidder) mới có quyền nạp tiền!");
    ...
    private static Response handleConfirmPayment(Request request, ClientHandler handler) {
        ...
        if (!(user instanceof Bidder)) return Response.error("Chỉ người mua (Bidder) mới có thể thực hiện thanh toán!");
    ...
    private static Response handleVerifyUsername(Request request) {
        ...
        if (user != null) return Response.ok(user);
        else return Response.error("Tên đăng nhập không tồn tại!");
    }
